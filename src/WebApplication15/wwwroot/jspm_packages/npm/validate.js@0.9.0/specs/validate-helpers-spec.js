  ...
  describe("validate", function() {
    ...
    describe('convertErrorMessages', function() {
      ...
      it("allow messages to contain %{value}", function() {
        ...
        expect(validate.format).toHaveBeenCalledWith("Foo foo %{value}", {value: "barfoo"});
      });
    });
    ...
    describe("processValidationResults", function() {
      ...
      it("supports multiple entries for the same attribute", function() {
        ...
        expect(pvr(results, {})).toEqual({name: ["Name foo", "Name bar", "Name baz"]});
      });
      it("the correct functions", function() {
        ...
        expect(validate.pruneEmptyErrors).toHaveBeenCalledWith("input", options);
        expect(validate.expandMultipleErrors).toHaveBeenCalledWith("pruned", options);
        expect(validate.convertErrorMessages).toHaveBeenCalledWith("expanded", options);
      });
    ...
    describe("flattenErrorsToArray", function() {
      it("flattens an array of errors objects to just the messages", function() {
        ...
        expect(validate.flattenErrorsToArray(input)).toEqual(["error 1", "error 2"]);
      });
    });
    ...
    describe("warn", function() {
      ...
      it("calls console.warn if defined", function() {
        window.console = {warn: jasmine.createSpy("warn")};
        validate.warn("Msg");
    ...
    describe("error", function() {
      ...
      it("calls console.error if defined", function() {
        window.console = {error: jasmine.createSpy("error")};
        validate.error("Msg");
    ...
    describe("getDeepObjectValue", function() {
      ...
      it("handles escapes properly", function() {
        ...
        expect(validate.getDeepObjectValue(attributes, "foo.bar.baz")).toBe(undefined);
        expect(validate.getDeepObjectValue(attributes, "foo\\.bar.baz")).toBe(3);
        expect(validate.getDeepObjectValue(attributes, "foo\\\\.bar.baz")).toBe(5);
        expect(validate.getDeepObjectValue(attributes, "\\foo\\\\.bar.baz")).toBe(5);
      });
    });
    ...
    describe("collectFormValues", function() {
      ...
      it("handles simple forms", function() {
        ...
        form.innerHTML = '' + '<input type="text" name="text" value="example text">' + '<input type="text" name="empty-text">' + '<input type="email" name="email" value="example@email.com">' + '<input type="password" name="password" value="password!">' + '<input type="checkbox" name="selected-checkbox" value="checkbox" checked>' + '<input type="checkbox" name="deselected-checkbox" value="checkbox">' + '<input type="date" name="date" value="2015-03-08">' + '<input type="hidden" name="hidden" value="hidden">' + '<input type="number" name="number" value="4711">' + '<input type="url" name="url" value="http://validatejs.org">' + '<input type="radio" name="single-checked-radio" value="radio" checked>' + '<input type="radio" name="single-unchecked-radio" value="radio">' + '<radiogroup>' + '  <input type="radio" name="checked-radio" value="radio1">' + '  <input type="radio" name="checked-radio" value="radio2" checked>' + '  <input type="radio" name="checked-radio" value="radio3">' + '</radiogroup>' + '<radiogroup>' + '  <input type="radio" name="unchecked-radio" value="radio1">' + '  <input type="radio" name="unchecked-radio" value="radio2">' + '  <input type="radio" name="unchecked-radio" value="radio3">' + '</radiogroup>' + '<select name="selected-dropdown">' + '  <option>' + '  <option value="option1">' + '  <option value="option2" selected>' + '</select>' + '<select name="unselected-dropdown">' + '  <option>' + '  <option value="option1">' + '  <option value="option2">' + '</select>' + '<textarea name="textarea-ignored" data-ignored>the textarea</textarea>' + '<textarea name="textarea">the textarea</textarea>';
      ...
      it("has an option to nullify empty and trim strings", function() {
        var form = document.createElement("form");
        form.innerHTML = '' + '<input type="text" name="normal" value="normal">' + '<input type="text" name="empty">' + '<input type="text" name="whitespace" value=" ">' + '<input type="text" name="trimmed" value=" foo ">';
      ...
      it("uses true/false for checkboxes without a value", function() {
        var form = document.createElement("form");
        form.innerHTML = '' + '<input type="checkbox" name="checked" checked>' + '<input type="checkbox" name="unchecked">';
        expect(validate.collectFormValues(form)).toEqual({
          checked: true,
          unchecked: false
        });
      ...
      it("accepts jquery elements", function() {
        ...
        expect(validate.collectFormValues($form)).toEqual({input: "foobar"});
      });
      ...
      it("handles empty and invalid numeric inputs", function() {
        var form = document.createElement("form");
        form.innerHTML = '' + '<input type="number" name="emptyNumber">' + '<input type="number" name="invalidNumber" value="abc">';
        expect(validate.collectFormValues(form)).toEqual({
          emptyNumber: null,
          invalidNumber: null
        });
    ...
    describe("cleanAttributes", function() {
      ...
      it("returns a copy of the attributes with only the whitelisted attributes", function() {
        ...
        expect(validate.cleanAttributes(input, {foo: true})).toEqual({foo: "foo"});
        expect(validate.cleanAttributes(input, {
          foo: true,
          bar: true
        })).toEqual({
          foo: "foo",
