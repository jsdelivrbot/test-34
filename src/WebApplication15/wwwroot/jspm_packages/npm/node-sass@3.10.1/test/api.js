...
(function(Buffer, process) {
  var assert = require('assert'),
      ...
      sassPath = process.env.NODESASS_COV ? require.resolve('../lib-cov') : require.resolve('../lib'),
  ...
  describe('api', function() {
    describe('.render(options, callback)', function() {
      ...
      it('should compile sass to css with file', function(done) {
        ...
        sass.render({file: fixture('simple/index.scss')}, function(error, result) {
      ...
      it('should compile sass to css with data', function(done) {
        ...
        var expected = read(fixture('simple/expected.css'), 'utf8').trim();
        sass.render({data: src}, function(error, result) {
          assert.equal(result.css.toString().trim(), expected.replace(/\r\n/g, '\n'));
          done();
        });
      });
      it('should NOT compile empty data string', function(done) {
        sass.render({data: ''}, function(error) {
      ...
      it('should throw error status 1 for bad input', function(done) {
        sass.render({data: '#navbar width 80%;'}, function(error) {
      ...
      it('should compile with include paths', function(done) {
        ...
        sass.render({
          data: src,
          includePaths: [fixture('include-path/functions'), fixture('include-path/lib')]
        }, function(error, result) {
          assert.equal(result.css.toString().trim(), expected.replace(/\r\n/g, '\n'));
          done();
        });
      ...
      it('should contain all included files in stats when data is passed', function(done) {
        ...
        var expected = [fixture('include-files/bar.scss').replace(/\\/g, '/'), fixture('include-files/foo.scss').replace(/\\/g, '/')];
        sass.render({
          data: src,
    ...
    describe('.render(importer)', function() {
      ...
      it('should respect the order of chained imports when using custom importers and one file is custom imported and the other is not.', function(done) {
        sass.render({
          ...
          importer: function(url, prev, done) {
            ...
            done({file: fixture('include-files/' + url + '.scss')});
          }
        }, function(err, data) {
      ...
      it('should should resolve imports depth first', function(done) {
        ...
        var expectedImportOrder = ['a', '_common', 'vars', 'struct', 'a1', 'common', 'vars', 'struct', 'b', 'b1'];
      ...
      it('should override imports with "file" as input and fires callback with file', function(done) {
        sass.render({
          file: fixture('include-files/index.scss'),
          importer: function(url, prev, done) {
            done({file: path.resolve(path.dirname(prev), url + (path.extname(url) ? '' : '.scss'))});
          }
        }, function(error, result) {
          assert.equal(result.css.toString().trim(), '/* foo.scss */\n/* bar.scss */');
          done();
      ...
      it('should override imports with "data" as input and returns file', function(done) {
        sass.render({
          ...
          importer: function(url) {
            return {file: path.resolve(path.dirname(fixture('include-files/index.scss')), url + (path.extname(url) ? '' : '.scss'))};
          }
        }, function(error, result) {
          assert.equal(result.css.toString().trim(), '/* foo.scss */\n/* bar.scss */');
          done();
      ...
      it('should override imports with "file" as input and returns file', function(done) {
        sass.render({
          file: fixture('include-files/index.scss'),
          importer: function(url, prev) {
            return {file: path.resolve(path.dirname(prev), url + (path.extname(url) ? '' : '.scss'))};
          }
        }, function(error, result) {
          assert.equal(result.css.toString().trim(), '/* foo.scss */\n/* bar.scss */');
          done();
      ...
      it('should override imports with "data" as input and fires callback with contents', function(done) {
        sass.render({
          data: src,
          importer: function(url, prev, done) {
            done({contents: 'div {color: yellow;}'});
          }
        }, function(error, result) {
          assert.equal(result.css.toString().trim(), 'div {\n  color: yellow; }\n\ndiv {\n  color: yellow; }');
          done();
      ...
      it('should override imports with "data" as input and returns contents', function(done) {
        sass.render({
          data: src,
          importer: function() {
            return {contents: 'div {color: yellow;}'};
          }
        }, function(error, result) {
          assert.equal(result.css.toString().trim(), 'div {\n  color: yellow; }\n\ndiv {\n  color: yellow; }');
          done();
    ...
    describe('.render(functions)', function() {
      ...
      it('should call custom function with multiple args', function(done) {
        sass.render({
          ...
          functions: {'foo($a, $b)': function(factor, size) {
      ...
      it('should work with custom functions that return data asynchronously', function(done) {
        sass.render({
          ...
          functions: {'foo($a)': function(size, done) {
              setTimeout(function() {
      ...
      it('should properly convert boolean when calling custom functions', function(done) {
        sass.render({
          ...
          functions: {'foo($a, $b)': function(a, b) {
      ...
      it('should properly convert lists when calling custom functions', function(done) {
        sass.render({
          ...
          assert.equal(result.css.toString().trim(), '.foo {\n  color: #fff; }\n\n.bar {\n  color: #fff; }\n\n.baz {\n  color: #fff; }');
          done();
        });
      });
      it('should properly convert maps when calling custom functions', function(done) {
        sass.render({
          data: '$test-map: foo((abc: 123, #def: true)); div { color: if(map-has-key($test-map, hello), #fff, #000); }' + 'span { color: map-get($test-map, baz); }',
          functions: {'foo($m)': function(map) {
      ...
      it('should properly convert null when calling custom functions', function(done) {
        sass.render({
          data: 'div { color: if(foo("bar"), #fff, #000); } ' + 'span { color: if(foo(null), #fff, #000); }' + 'table { color: if(bar() == null, #fff, #000); }',
          functions: {
            'foo($a)': function(a) {
      ...
      it('should always map null, true and false to the same (immutable) object', function(done) {
        ...
        sass.render({
          ...
          functions: {
            foo: function(a) {
              assert.strictEqual(a, sass.TRUE, 'Supplied value should be the same instance as sass.TRUE');
              assert.strictEqual(sass.types.Boolean(true), sass.types.Boolean(true), 'sass.types.Boolean(true) should return a singleton');
              assert.strictEqual(sass.types.Boolean(true), sass.TRUE, 'sass.types.Boolean(true) should be the same instance as sass.TRUE');
              counter++;
              return sass.types.String('foo');
            },
            bar: function(a) {
              assert.strictEqual(a, sass.NULL, 'Supplied value should be the same instance as sass.NULL');
            ...
            baz: function(a) {
              assert.strictEqual(a, sass.FALSE, 'Supplied value should be the same instance as sass.FALSE');
              assert.throws(function() {
                return new sass.types.Boolean(false);
              }, /Cannot instantiate SassBoolean/);
              assert.strictEqual(sass.types.Boolean(false), sass.types.Boolean(false), 'sass.types.Boolean(false) should return a singleton');
              assert.strictEqual(sass.types.Boolean(false), sass.FALSE, 'sass.types.Boolean(false) should return singleton identical to sass.FALSE');
              counter++;
    ...
    describe('.render({stats: {}})', function() {
      ...
      it('should provide a start timestamp', function(done) {
        sass.render({file: fixture('include-files/index.scss')}, function(error, result) {
          assert(!error);
      ...
      it('should contain an array of all included files', function(done) {
        var expected = [fixture('include-files/bar.scss').replace(/\\/g, '/'), fixture('include-files/foo.scss').replace(/\\/g, '/'), fixture('include-files/index.scss').replace(/\\/g, '/')];
        sass.render({file: fixture('include-files/index.scss')}, function(error, result) {
          assert(!error);
      ...
      it('should state `data` as entry file', function(done) {
        sass.render({data: read(fixture('simple/index.scss'), 'utf8')}, function(error, result) {
    ...
    describe('.renderSync(functions)', function() {
      it('should call custom function in sync mode', function(done) {
        var result = sass.renderSync({
          ...
          functions: {'cos($a)': function(angle) {
      ...
      it('should return a list of selectors after calling the headings custom function', function(done) {
        var result = sass.renderSync({
          ...
          functions: {'headings($from: 0, $to: 6)': function(from, to) {
              var i,
                  f = from.getValue(),
                  t = to.getValue(),
    ...
    describe('.renderSync({stats: {}})', function() {
      var start = Date.now();
      var result = sass.renderSync({file: fixture('include-files/index.scss')});
      it('should provide a start timestamp', function(done) {
        assert.strictEqual(typeof result.stats.start, 'number');
        assert(result.stats.start >= start);
        done();
      ...
      it('should contain an array of all included files', function(done) {
        var expected = [fixture('include-files/bar.scss').replace(/\\/g, '/'), fixture('include-files/foo.scss').replace(/\\/g, '/'), fixture('include-files/index.scss').replace(/\\/g, '/')].sort();
      ...
      it('should state `data` as entry file', function(done) {
        var result = sass.renderSync({data: read(fixture('simple/index.scss'), 'utf8')});
        assert.equal(result.stats.entry, 'data');
        done();
      });
      it('should contain an empty array as includedFiles', function(done) {
      ...
      describe('on unsupported environment', function() {
        describe('with an unsupported architecture', function() {
          ...
          beforeEach(function() {
            ...
            Object.defineProperty(process, 'arch', {get: function() {
                return 'foo';
              }});
          });
          afterEach(function() {
        ...
        describe('with an unsupported platform', function() {
          ...
          beforeEach(function() {
            ...
            Object.defineProperty(process.versions, 'modules', {get: function() {
                return 'baz';
              }});
          });
          afterEach(function() {
