...
(function(process) {
  ...
  function afterBuild(options) {
    ...
    var target = path.join(__dirname, '..', 'build', options.debug ? 'Debug' : process.config.target_defaults ? process.config.target_defaults.default_configuration : 'Release', 'binding.node');
  ...
  function build(options) {
    installGitDependencies(options, function(err) {
      ...
      var proc = spawn(process.execPath, args, {stdio: [0, 1, 2]});
